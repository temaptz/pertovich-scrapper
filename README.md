...
|----------------------|-------------------------------------------------------|--------------|
| `DOMAIN`             | Целевой домен каталога (**обязательная**)             | —            |
...
Образ собирается [многоэтапно](Dockerfile): системные библиотеки и шрифты → зависимости Playwright → браузер Camoufox → runtime от non-root пользователя `camouser`.

**1.** Создать `.env` (см. [Быстрый старт](#быстрый-старт)). Для Docker обязательно:
...
- `shm_size: '2gb'` — разделяемая память для стабильной работы Firefox (дефолтные 64 МБ приводят к крахам рендеринга)
- `init: true` — корректная обработка сигналов и очистка зомби-процессов
- `volumes: ./temp:/app/temp` — проброс результата (`catalog.json`) и персистентного профиля браузера на хост
- `env_file: .env` — передача параметров окружения
